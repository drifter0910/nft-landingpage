...
export default function Creators() {
  return (
    <div className="py-10 md:py-20" data-aos="fade-up" data-aos-duration="2000">
      ...
      <Marquee pauseOnHover={true} gradient={false} className="mt-5 md:mt-10 py-4">
        {data.map((item) => {
          return (
            <div className="flex items-center gap-3  mr-4 md:mr-12 cursor-pointer hover:scale-105 transition-all">
              <img src={`/img/${item.image}`} alt="" className="rounded-full w-1/4 md:w-auto" />
              <div className="text-tertiary">
                <p className="font-primary text-lg md:text-2xl font-semibold">{item.name}</p>
